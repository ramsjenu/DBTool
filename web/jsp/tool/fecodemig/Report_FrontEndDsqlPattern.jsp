...
<script>
 ...
 function openFullReport(){
		document.form1.action = "<%=filePath%>/tool/feCodeMigrationReports.action?submitMode=feDsqlPatternReportDownload&projectId=<%=projectId%>&analysisType=<%=analysisType%>&seqNo=<%=seqNo%>&migrationMode=<%=migrationMode%>","repot1","menubar=0,resizable=0,width=850,height=500";
		document.form1.submit();
	 }
		...
		<table width="100%" bgcolor="#f4e7bd" border=0>
			...
			<tr width="100%">
				<td width="100%" class="pageheadernoWidth">&nbsp;
					&nbsp;&nbsp;&nbsp;Front End DSQL Pattern Report</td>
			</tr>
    ...
    <TD id=visibleTotal>…</TD></TR></TFOOT> -->
						<THEAD>
							<TR class="tableheader" align="left">
								...
								<TH class=sortable-currency>Line Num</TH>
								<TH class=sortable-currency>Embedded SQL</TH>
								<TH class=sortable-currency>SQL Invoked Line Num</TH>
							</TR>
						</THEAD>
						<TBODY>
							...
							<tr class="<%=lCssClass%>" align="left">
								<td width="4%"><%=(lSno) %></td>
								<td><%=lSourceFilePath %></td>
								<td><%=lDynamicSQLDataDTO.getSourceFileName().trim() %></td>
								<td><%=(String)lLineNumsList.get(j) %></td>
								<td><%=lDSqlQueryArr[j] %></td>
								<td><%=lDynamicSQLDataDTO.getInvokedLineNum() %></td>
							</tr>
