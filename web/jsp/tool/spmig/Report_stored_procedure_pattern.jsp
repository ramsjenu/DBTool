...
<script>
 ...
 function openFullReport(){
		document.form1.action = "<%=filePath%>/tool/spMigrationReports.action?submitMode=spPatternResultReportDownLoad&projectId=<%=projectId%>&analysisType=<%=analysisType%>&seqNo=<%=seqNo%>","repot1","menubar=0,resizable=0,width=850,height=500";
		document.form1.submit();
	 }
		...
		<table width="100%" bgcolor="#f4e7bd" border=0>
			...
			<tr width="100%">
				<td width="100%" class="pageheadernoWidth">&nbsp;
					&nbsp;&nbsp;&nbsp;Stored Procedure Pattern Report</td>
			</tr>
				...
				<td align="right">&nbsp;
					<div id="report1_link">
						&nbsp; <a class="moreproject2" style="cursor: hand"
							onclick="openFullReport();"><u><font color="blue">Download
									Pattern Analysis Detail Report in Excel Format </font></u> </a>
				</td>
			</tr>
    ...
    <TD id=visibleTotal>…</TD></TR></TFOOT> -->
						<THEAD>
							<TR class="tableheader" align="left">
								...
								<TH class=sortable-text>Stored Procedure Name</TH>
								<TH class=sortable-text>Line No</TH>
								<TH class=sortable-currency>Category</TH>
								<TH class=sortable-currency>Pattern Id</TH>
								<TH class=sortable-currency>Pattern Desc</TH>
								<TH class=sortable-currency>No. Sub Statements</TH>
							</TR>
						</THEAD>
						<TBODY>
						...
						%>
							...
							<tr class="<%=lCssClass%>" align="left">
								<td width="4%"><%=(i+1) %></td>
								<td width="26%"><%=lSPPatternAnalysisReportDTO.getProcName() %></td>
								<td width="4%"><%=lSPPatternAnalysisReportDTO.getStatementNo() %></td>
								<td width="10%"><%=lSPPatternAnalysisReportDTO.getCategory() %></td>
								<td width="14%"><%=lSPPatternAnalysisReportDTO.getPatternId() %></td>
								<td width="24%"><%=lSPPatternAnalysisReportDTO.getPatternDesc() %></td>
								<td width="9%"><%=lSPPatternAnalysisReportDTO.getQueryCount() %></td>
								<%-- <td width="9%"><% if(lSPPatternAnalysisReportDTO.getStatement().trim().length() >25){
													out.println((lSPPatternAnalysisReportDTO.getStatement().trim()).substring(0,25));
												}else{
													out.println((lSPPatternAnalysisReportDTO.getStatement().trim()));
												}
											%>
							</td> --%>
							</tr>
