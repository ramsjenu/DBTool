...
<script>
	...
	function openPopUpReportSummary(){
	//window.open ("success_summary_report.html","repot1","menubar=0,resizable=0,width=750,height=500");
		window.open ("<%=filePath%>/tool/spMigrationReports.action?submitMode=compareSummaryReport&projectId=<%=projectId%>&migrationMode=<%=migrationMode%>&seqNo=<%=seqNo%>","detailedSummaryReport","scrollbars=1,menubar=0,resizable=1,width=850,height=500");
	}
	function openPopUpCategoryCountReport(){
		var randomno=Math.floor((Math.random()*100)+1);
		//window.open ("success_summary_report.html","repot1","menubar=0,resizable=0,width=750,height=500");
			window.open ('<%=filePath%>/tool/spMigrationReports.action?submitMode=mismatchCategorySummeryReport&projectId=<%=projectId%>&migrationMode=<%=migrationMode%>&seqNo=<%=seqNo%>','detailedSummaryReport'+randomno,'scrollbars=1,menubar=0,resizable=1,width=850,height=500');
		}
	
	function openPopUpReportGap(){
		//window.open ("gap_analysis_report.html","repot1","menubar=0,resizable=0,width=750,height=500");
		window.open ("<%=filePath%>/tool/spMigrationReports.action?submitMode=compareSummaryReportSpWiseCount&projectId=<%=projectId%>&migrationMode=<%=migrationMode%>&seqNo=<%=seqNo%>","detailedSummaryReport3","scrollbars=1,menubar=0,resizable=1,width=850,height=500");
	}
	
	 function openPopUpReport(){
		var randomno=Math.floor((Math.random()*100)+1); 
		window.open ('<%=filePath%>/tool/spMigrationReports.action?submitMode=detailedSummaryReport&projectId=<%=projectId%>&migrationMode=<%=migrationMode%>&seqNo=<%=seqNo%>','detailedSummaryReport'+randomno,'scrollbars=1,menubar=0,resizable=1,width=850,height=500');
	...
	function changeProjectName(){
		...
		document.form1.action='<%=filePath%>'+"/tool/analyseGaps.action?";
	...
	<%-- 
		...
		for(var x=0;x < '<%=procNameList.size()%>';x++){
			...
			if(funcRet == true){
				tdInnerHtmlValue += "<input type=\"checkbox\" name=\"partialSP"+x+"\" id=\"partialSP"+x+"\"/> ";
				tdInnerHtmlValue += "&nbsp;"+document.getElementById("hiddenPartialSPName"+x).value;
				tdInnerHtmlValue += "<input type=\"hidden\" name=\"partialSPName"+x+"\" id=\"partialSPName"+x+"\"  value=\""+document.getElementById("hiddenPartialSPName"+x).value+"\"/> ";
				addRowInnerHTML(tableName ,tdInnerHtmlValue,"tablerowodd");
			}
				...
				<td width="60%" valign="top">
					...
					<table width="100%" border=0 cellspacing="2" cellpadding="2"
						...
						<%-- <tr height="25%">
		      			...
		      			<td  nowrap width="75%" >
		      				<select class="input1" name="analysisType">
		      					<option>--Select--</option>
		      					<option <%if("SOURCE".equalsIgnoreCase(analysisType)){out.println("selected");} %> value="SOURCE">Source</option>
		      					<option <%if("TARGET".equalsIgnoreCase(analysisType)){out.println("selected");} %> value="TARGET">Target</option>      					
		      				 </select>
		      			</td>
		      		</tr>  --%>
						...
						<tr>
							...
							<td><select class="input1" id="migrationMode"
								...
								onchange="return changeAnalysisMode(this.value)">
									...
									<option
										<% if("Partial".equalsIgnoreCase(migrationMode)){out.println("selected");} %>
						...
						<tr>
							...
							<td colspan="1" align="left"><input type="button"
								...
								onclick="return submitIdentifyPattern();" disabled="disabled" style="background-color:gray ;color: #000101;cursor: text; ">
						...
						<tr>
							<td colspan="2" align="center"><div id="report1_link"
									...
									&nbsp;
									<!-- <a class="moreproject" style="cursor:hand" onclick="openPopUpReportGap();"><u><font color="blue">Click	Here to open the Stored Procedure wise MisMatch Summary Report</font></u> </a>
									 
				</br>
				</br>
				<a class="moreproject" style="cursor:hand" onclick="openPopUpReportSummary();"><u><font color="blue">Click	Here to open the MisMatch Summary Report</font></u> </a>
				</br>
				</br>
				 -->
									<a class="moreproject2" style="cursor: hand" 
										onclick="return openPopUpReport();"><u><font color="blue" style="text-align: left">Detailed
												Comparison Report</font></u> </a> <br/> <br/> <a class="moreproject2"
										style="cursor: hand;"
										onclick="return openPopUpCategoryCountReport();"><u><font
											color="blue" style="text-align: left">Mismatch Category Count Report</font></u> </a>
