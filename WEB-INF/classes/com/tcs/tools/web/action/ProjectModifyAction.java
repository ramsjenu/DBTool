...
public class ProjectModifyAction extends ActionSupport /*implements ServletRequestAware,ServletResponseAware */ {
	...
	if("save".equalsIgnoreCase(getSubmitMode())){
		...
		String lRunSeq = lProjectCreationDAO.getRunSeq(getProjectName());
			...
			if(getTargetSPUpFileFileName() != null && (!"".equals(getTargetSPUpFileFileName()))){
				if (FileUploadDownloadUtility.validateFile(
					...
					msgValue.append("Invalid TargetFile Type");
					retValue = "error";
				}else{
					//copy file to server path		
		...
		//	String lSourceDbType = "SYSBASE";
			...
			else if("ORACLE".equalsIgnoreCase(getTargetDBType())) {
			...
			if(getSourceSPUpFileFileName() != null && (!"".equals(getSourceSPUpFileFileName()))){
				...
				ToolsUtil.prepareInsertStatusMsg( getProjectName(), ToolConstant.CREATED_BY, lCurState,lStausMsg, "Heading",new Timestamp(System.currentTimeMillis()),lConnection);
			...
			if(getSourceSPUpFileFileName() != null && (!"".equals(getSourceSPUpFileFileName()))){
				msgValue.append("Files Uploaded SuccessFully....");
			}else{
	 ...
	 if("reset".equalsIgnoreCase(getSubmitMode())){
   	...
   	// return SUCCESS;
		...
		if(!"".equals(retValue)){
			if(retValue.trim().equalsIgnoreCase("error")){
				setMsgToJsp("");
			}
			return retValue;
		}
